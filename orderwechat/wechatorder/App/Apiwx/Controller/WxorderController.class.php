...
//获取当前订单信信息
	public function getGoodDetails1111(){
		$g_id=I('g_id');
		if ($g_id) {
			$condition['g_id']=$g_id;
			$goodsList=M('goods')->where($condition)->field('gt_name,price,pic_url,introduce')->find();
			$url=C('URL');
			$goodsList['pic_url']=$url.$value['pic_url'];
	...
	public function submitOrderByShop()
	{
		if(!IS_POST){
			...
			$t=strtotime($nowTime);
			$condition1['sub_time']=array('gt',$t);
			$num_wait=M('order')->where($condition1)->order('sub_time desc')->find();
			if($num_wait['wait_num']){
			if(mb_substr($num_wait['wait_num'],0,1)=='T'){
			$new_waitnum=mb_substr($num_wait['wait_num'],1,3);
			$count1=intval($new_waitnum);
			if($count1!=0){
				$count1++;
				$wait_num1=sprintf("%03d",$count1);
				$wait_num='T'.$wait_num1;
			}
		}
		else{
			$count1=intval($num_wait['wait_num']);
			if($count1!=0){
				$count1++;
				$wait_num1=sprintf("%03d",$count1);
				$wait_num='T'.$wait_num1;
			...
			$content = "<FS2><center>**".$shop_name."**</center></FS2>";
			$content .= str_repeat('.', 32);
			$content .= "<FS2><center>--线下支付--</center></FS2>";
			$content .= "<FS><center>--已支付--</center></FS>";
			$content .= "<FS><center>付款码:".strval($data['wait_num'])."</center></FS>";
			$content .= "订单时间:". date("Y-m-d H:i",$data['sub_time']) . "\n";
			$content .= "订单编号:".$data['o_num']."\n";
			...
			foreach ($list as $key => $value) {
				...
				$temp['pack_num']=$value['pack_num'];
				if($temp['pack_num']==0)
				{
					$content.="<tr><td>".$temp['goods_name']."</td><td>x".strval($temp['count'])."</td><td>".strval($temp['sub_total'])."</td></tr>";
				}
				else{
					$content.="<tr><td>".$temp['goods_name']."</td><td>x".strval($temp['count'])."(打包数x".strval($temp['pack_num']).")".strval($temp['sub_total'])."</td></tr>";
				}
			...
			$content .= "</table>";
			$content .= str_repeat('.', 32);
			$content .= "小计:￥".strval($data['total_money'])."\n";
			$content .= "折扣:￥0 \n";
			$content .= str_repeat('*', 32);
			$content .= "订单总价:￥".strval($data['total_money'])." \n";
			$content .= "<FS2><center>**完**</center></FS2>";
			$condition['s_id']=I('shop_id');
			$is_print=M('store')->where($condition)->getField('is_print');
			//$this->ajaxReturn($is_print);
			if($is_print==1){
			$this->print_order($data['o_num'],$content,$shop_id);
	...
	public function getOrderSub(){
		...
		if(I('openid')){
			...
			$orderList=M('order')->where($condition)->field('o_id,shop_id,goods_count,total_money,wait_num,order_status')->order('sub_time desc')->select();
			//根据订单中shop_id查找商店信息
	...
	public function getOrderPay(){
		...
		if(I('openid')){
			...
			$orderList=M('order')->where($condition)->field('o_id,shop_id,goods_count,total_money,wait_num,order_status')->order('pay_time desc')->select();
			//根据订单中shop_id查找商店信息
			foreach ($orderList as $k => $value) {
				$storeId=$orderList[$k]['shop_id'];
				$storeInfo=M('store')->where("s_id=$storeId")->field('s_storename,pic_url')->select();
	...
	public function getOrderDone(){
		...
		if(I('openid')){
			...
			$condition['user_openid']=$openid;
			$condition['order_status']=2;
			$orderList=M('order')->where($condition)->field('o_id,shop_id,goods_count,total_money,wait_num,order_status')->order('pay_time desc')->limit($min,$callback)->select();
			//根据订单中shop_id查找商店信息
			foreach ($orderList as $k => $value) {
				$storeId=$orderList[$k]['shop_id'];
				$storeInfo=M('store')->where("s_id=$storeId")->field('s_storename,pic_url')->select();
	...
	public function cancelPay()
	{
		...
		if(M('order')->where($condition)->getField('order_status')==3){
			$condition1['s_id']=M('order')->where($condition)->getField('shop_id');
			$data1=M('store')->where($condition1)->find();
			$data=M('order')->where($condition)->find();
			$shop_name=$data1['s_storename'];
			$content = "<FS2><center>**".$shop_name."**</center></FS2>";
			$content .= str_repeat('.', 32);
			$content .= "<FS2><center>--线下支付--</center></FS2>";
			$content .= "<FS><center>--取消订单--</center></FS>";
			$content .= "<FS><center>付款码:".strval($data['wait_num'])."</center></FS>";
			$content .= "订单时间:". date("Y-m-d H:i",$data['sub_time']) . "\n";
			$content .= "订单编号:".$data['o_num']."\n";
			$content .= str_repeat('*', 14) . "商品" . str_repeat("*", 14);
			$content .= "<table>";
			$detail1=json_decode($data['detail'],true);
			$temp=array();
			...
			$content .= "订单总价:￥".strval($data['total_money'])." \n";
			$content .= "<FS2><center>**完**</center></FS2>";
			$is_print=M('store')->where($condition1)->getField('is_print');
			if($is_print==1){
			$this->print_cancelorder($content,$condition1['s_id'],$data1['printername']);
			//$this->ajaxReturn($result);
			}
	    	...
	    	//打印小票(小程序端绑定测试版)
	public function print_cancelorder($content,$shop_id,$machine_code){
	...
	//打印小票(接口调用版)
	public function print_order($data,$content,$shop_id){
		//$this->ajaxReturn($shop_id);
		$condition['s_id']=$shop_id;
		$machine_code=M('store')->where($condition)->getField('printername');
		//$this->ajaxReturn($machine_code);
		$post_url='https://open-api.10ss.net/print/index';
		$timestamp=time();
		$post_data=array(
                    'client_id' =>C('CLIENT_ID'),
                    ...
                    //要修改机器码。
                    'machine_code'=>$machine_code,
                    //'machine_code'=>'4004573668',//死数据，待改
                    'origin_id'=>$data,
                    'content'=>$content,
                    'sign'=>getSign($timestamp),
                    'id'=>uuid4(),
